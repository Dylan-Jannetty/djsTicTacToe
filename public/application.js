...
var alertCurrentPlayer = function alertCurrentPlayer() {
...
var addToBoard = function addToBoard(event) {
  var position = $(event.target).attr('data-cell'); // get id of box you clicked on
  gameBoard[position] = currentPlayer;
};

// write a function that only adds to the board if the spot is free
var spotTaken = function spotTaken() {
...
var checkDraw = function checkDraw() {
  var result = gameBoard.some(isEmpty);
  if (result === false) {
...
var onNewGame = function onNewGame(event) {
  event.preventDefault();
  api.newGame().then(ui.startNewGame).catch(ui.newGameError);
  clearBoard();
  if (currentPlayer === 'O') {
    return switchTurns();
  }
...
var onGetGameData = function onGetGameData(event) {
  event.preventDefault();
  api.getGameData().then(ui.gameDataSuccess).catch(ui.gameDataError);
};

var addHandlers = function addHandlers() {
  $('.box').on('click', function (event) {
    var position = $(event.target).data('cell');
    var obj = void 0;
    // if game is over
    ...
    api.updateGame(obj).then(ui.onUpdateGameSuccess).catch(ui.onUpdateGameFailure);
  });
...
var startNewGame = function startNewGame(data) {
...
var onUpdateGameSuccess = function onUpdateGameSuccess(data) {
  store.game.id = data.game.id;
};
var onUpdateGameFailure = function onUpdateGameFailure(data) {
  store.game.id = data.game.id;
};
var gameDataSuccess = function gameDataSuccess(data) {
  $('#message').text('games played: ' + data.games.length);
  $('#message').css('background-color', 'Green');
};
var gameDataError = function gameDataError(data) {
...
module.exports = {
  ...
  gameDataSuccess: gameDataSuccess,
...
var getGameData = function getGameData(data) {
  return $.ajax({
    url: config.apiUrl + '/games',
    method: 'GET',
    headers: {
